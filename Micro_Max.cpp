...
//************************************  GAME OVER
void gameOver(String dinner) {
  state = 9;
  winner = dinner;
  memcpy(b, init_board_config, sizeof(b));
  mn = 1;
  game_finished = 0;
}
...
void AI_HvsH() {
  ...
  if (state == 7) {  //blancs
    Serial.print(mn);
    Serial.print(". ");
  } else if (state == 8) Serial.print("   ");  //Noirs
  ...
  if (state == 8) {
    ...
    if (!(r > -I + 1)) {
      gameOver("blancs");
      Serial.println("Lose2 ");
    }
    Serial.println("");
    serialBoard();
  }
  if (state == 7) mn++; /* Next move */
}

//***************************  AI COMPUTER PLAYER
void AI_HvsC(uint8_t level) {
  inputString += mov;
  Serial.print(mn);
  Serial.print(". ");
  // Serial.print(inputString.substring(0, 4));
  ...
  K = *c - 16 * c[1] + 799, L = c[2] - 16 * c[3] + 799; /* parse entered move */
  N = 0;
  T = bot_level[level];         /* T=Computer Play strength */
  bkp();                    /* Save the board just in case */
  r = D(-I, I, Q, O, 1, 3); /* Check & do the human movement */
  if (!(r > -I + 1)) {
    Serial.println("Lose3 ");
    game_finished = 2;
  }
  if (k == 0x10) { /* The flag turn must change to 0x08 */
    Serial.println("No valid move");
  ...
  if (!(r > -I + 1)) {
    Serial.println("Lose4");
    game_finished = 1;    
  }
  strcpy(lastM, c); /* Valid ARDUINO movement */
  r = D(-I, I, Q, O, 1, 3);
  if (!(r > -I + 1)) {
    Serial.println("Lose5");
    game_finished = 2;    
  }
